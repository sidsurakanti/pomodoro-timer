...
function main() {
	...
	display.addEventListener("timer-end", () => {
		...
		// plays timer end music for 12.5 seconds
		timerAudio.play();
		let audioInterval = setInterval(() => {
			if (timerAudio.currentTime > 12.5) {
				timerAudio.pause()
	...
	// resets <timerDisplay> to <startTime> and pauses any <timerAudio>
	resetButton.addEventListener("click", () => {
		if (interval) {
			const startTime = localStorage.getItem("plTime")
